...
- [x] 增加 `admin / user` 最小 RBAC
- [x] Prompt 管理接口统一限制为管理员
- [x] 增加 Prompt 版本和单 active 数据库约束
- [x] 增加受控 `create-admin` CLI
- [x] 文档上传分块落盘并限制大小、扩展名和 MIME
- [x] 数据库失败或上传超限时清理不完整文件
- [x] 文档处理使用行锁拒绝重复并发任务
- [x] Celery 增加 late ack、有限重试、退避和执行超时
- [x] 增加后端、前端和容器构建 CI 门禁
- [x] 增加面试亮点与演示路径文档
