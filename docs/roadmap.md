...
- 组件按业务场景拆分，不做通用后台框架。
- 可以用 `@tanstack/react-query`、`react-hook-form`、`zod` 这类成熟依赖减少手写状态和表单代码。
- 暂不引入复杂全局状态库、大型组件库或多页路由。

### P2: 建立 pgvector 多租户质量闭环
...
当前多租户产品化后端能力已经基本具备，下一阶段重点是前端最小产品闭环、产品化运维能力和真实数据质量基线。
